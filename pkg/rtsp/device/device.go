...
import (
	...
	"github.com/kubeedge/mappers-go/pkg/rtsp/configmap"
	"github.com/kubeedge/mappers-go/pkg/rtsp/driver"
	"github.com/kubeedge/mappers-go/pkg/rtsp/globals"
	proxy "github.com/majoyz/rtsp-simple-proxy"
	"k8s.io/klog/v2"
)
...
func setVisitor(visitorConfig *configmap.RtspVisitorConfig, twin *common.Twin, client *driver.RtspClient) {
	if twin.PVisitor.PProperty.AccessMode == "ReadOnly" {
		klog.V(1).Info("Visit readonly.")
		return
	}
...
func onMessage(client mqtt.Client, message mqtt.Message) {
	...
	for twinName, twinValue := range delta.Delta {
		...
		var visitorConfig configmap.RtspVisitorConfig
		...
		setVisitor(&visitorConfig, &dev.Instance.Twins[i], dev.RtspClient)
	}
}

// initRtsp initialize modbus client
func initRtsp(protocolConfig configmap.RtspProtocolConfig) (client *driver.RtspClient, err error) {
	if protocolConfig.ConfigData.IP != "" {
		rtsp := driver.Rtsp{User: string(protocolConfig.ConfigData.User),
			Password: protocolConfig.ConfigData.Password,
			IP:       protocolConfig.ConfigData.IP,
			Port:     int(protocolConfig.ConfigData.Port),
			Path:     protocolConfig.ConfigData.Path,
			Fullpath: protocolConfig.ConfigData.FullPath,
			Timeout:  5 * time.Second}
		client, _ = driver.NewClient(rtsp)
	} else {
		return nil, errors.New("No rtsp ip found")
	}
	return client, nil
}

// initTwin initialize the timer to get twin value.
func initTwin(dev *globals.RtspDev) {
	for i := 0; i < len(dev.Instance.Twins); i++ {
		twinData := TwinData{Client: dev.RtspClient,
...
func initData(dev *globals.RtspDev) {
...
func initGetStatus(dev *globals.RtspDev) {
	getStatus := GetStatus{Client: dev.RtspClient,
...
func start(dev *globals.RtspDev) {
	var protocolCommConfig configmap.RtspProtocolCommonConfig
	if err := json.Unmarshal([]byte(dev.Instance.PProtocol.ProtocolCommonConfig), &protocolCommConfig); err != nil {
		klog.Error(err)
		return
	}

	var protocolConfig configmap.RtspProtocolConfig
	if err := json.Unmarshal([]byte(dev.Instance.PProtocol.ProtocolConfigs), &protocolConfig); err != nil {
		klog.Error(err)
		return
	}

	client, err := initRtsp(protocolConfig)
	if err != nil {
		klog.Error(err)
		return
	}
	dev.RtspClient = client
	...
	defer f.Close()
	if err != nil {
		klog.Error("Open or create conf.yaml failed: ", err)
	}
	write := bufio.NewWriter(f)
	tmps := "streams:\n"
	write.WriteString(tmps)
	tmps = "  " + dev.Instance.Name + ":\n"
	write.WriteString(tmps)
	tmps = "    url: " + dev.RtspClient.Client.Fullpath + "\n"
	write.WriteString(tmps)
	tmps = "    protocol: tcp"
	write.WriteString(tmps)
	write.Flush()
	...
	// tmp := []string{""}
	_, err = proxy.NewProgram(os.Args[1:])
	if err != nil {
		klog.Info("Rtsp connected.")
	} else {
		klog.Error(err)
	}
